...
public class DateUtils {
	...
	public String getYYYYMMDDByDate(Date date) throws Exception {
	...
	public Date getDateByNoneStructure4News(String publish_time_string)
			throws Exception {
		if (StringUtils.isEmpty(publish_time_string)) {
			return null;
		}
	...
	public static String formatWebDateString(String dateString) {
		...
		dateString = dateString
				.replaceAll("今天", DateUtils.getTodayFomatString());
		dateString = dateString.replaceAll("昨天",
				DateUtils.getYesterdayStringByLine());
		...
		if (len >= 3) {
			if (len == 3) {
				...
				if (dateString.contains("-")) {// 说明是日期格式
					...
					return DateUtils.getTodayFomatString() + " " + "00:00:00";
				}
			} else if (len == 5) {
				// 对年份的处理
				if (arr[0].length() == 2) {
	...
	/**
	 * 得到两个日期之间的所有的date值数组,输入和返回格式为2014-01-31
	 * 
	 * @param beginDate
	 * @param endDate
	 * @return
	 */
	public List<String> getAllDateArray(String beginDate, String endDate)
			throws Exception {
		// String[] begin_dt_array = beginDate.split("-");
		// String[] end_dt_array = beginDate.split("-");
		if (beginDate.length() != 8 || endDate.length() != 8) {
			throw new Exception("输入的日期格式不正确，请检查!");
		}
		...
		long begin_date_long = getDateByNoneStructure4News(beginDate).getTime();
		long end_date_long = getDateByNoneStructure4News(endDate).getTime();

		long unit_day_long = 24 * 60 * 60 * 1000;
		for (int i = 0; i < 1000; i++) {
			begin_date_long = begin_date_long + unit_day_long
					* (i == 0 ? 0 : 1);
			if (begin_date_long > end_date_long) {
				break;
			}
			resultList.add(getYYYYMMDDByDate(new Date(begin_date_long)));
		}
