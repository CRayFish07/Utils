...
public final class HttpClientUtil {
    ...
    public static String sendGet(String url, int contimeout,int sotimeout) {
		
    	DefaultHttpClient httpClient = new DefaultHttpClient();
		httpClient.getParams().setParameter(CoreConnectionPNames.CONNECTION_TIMEOUT, contimeout); // 连接超时
		httpClient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT, sotimeout); // 数据传输超时
    ...
    public static String sendPostWithEntity(String url, HttpEntity entity, Map<String, String> headerMap)
            throws Exception {
    ...
    private static HttpClient getHttpClient() {
        if (httpClient == null) {
            ...
            HttpConnectionParams.setConnectionTimeout(defaultHttpClient.getParams(), 3000); // 建立连接3秒超时
            HttpConnectionParams.setSoTimeout(defaultHttpClient.getParams(), 8000); // 读取返回数据8超时
            // 增加gzip支持
